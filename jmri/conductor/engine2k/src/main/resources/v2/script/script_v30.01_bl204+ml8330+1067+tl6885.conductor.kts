...
data class _SP_Data(
    ...
    val B321_maxSecondsOnBlock: Int = 2*60,
)
...
val Freight_Route = ML_Route.sequence {
    ...
    val B321_fwd = node(B321) {
        ...
        onEnter {
            ...
            after (SP_Data.Delay_Up_Slow) then {
                ...
                // Don't use SP_Data.Delay_Up_Stop... instead crawl till we hit B330
            }
        }
    }
    ...
    val B330_fwd = node(B330) {
        minSecondsOnBlock = 0
        onEnter {
            // Stop immediately when reaching B330.
            FR.horn()
            FR.stop()
            FR.bell(Off)
    ...
    sequence = listOf(B311_start, B321_fwd, B330_fwd, B321_rev, B311_rev)
}
